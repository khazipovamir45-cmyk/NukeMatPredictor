# NukeMatPredictor

Веб-сайт для прогнозирования изменения прочности конструкционных материалов под воздействием нейтронного облучения и высоких температур.

---

## О проекте

**NukeMatPredictor** — это информационно-расчетный веб-сайт, разработанный в рамках учебного курса. Сайт позволяет специалистам в области ядерной энергетики и материаловедения выполнить расчет на основе введённых параметров.

---

## Функциональность

- **Главная страница** — описание проекта и кнопка перехода к расчёту.
- **Страница формы** — ввод параметров расчёта (температура, доза облучения и так далее).
- **Страница результатов** — таблица материалов с рассчитанной прочностью, график зависимости прочности от дозы облучения, график зависимости прочности от температуры
- **База данных** — 200 материалов с характеристиками.

---

## Быстрый старт

### Требования

- Python 3.8 или выше
- pip (менеджер пакетов Python)

### Установка и запуск

**1. Клонировать репозиторий:**
```bash
git clone https://github.com/ваш-username/NukeMatPredictor.git
cd NukeMatPredictor
```

**2. Создать и активировать виртуальное окружение:**
```bash
python -m venv venv
source venv/bin/activate      # для macOS/Linux
venv\Scripts\activate         # для Windows
```

**3. Установить зависимости:**
```bash
pip install -r requirements.txt
```

**4. Открыть в браузере: https://nukematpredictor.onrender.com

---

## Структура проекта
```
NukeMatPredictor/
├── app.py                    # Основной файл Flask (маршруты)
├── database.py               # Работа с базой данных SQLite
├── model.py                  # KNN-модель подбора материалов
├── requirements.txt          # Зависимости Python
├── nuclear_materials.csv     # Исходные данные (200 материалов)
├── scaler.pkl                # Обученный StandardScaler
├── materials_list.pkl        # Список материалов (joblib)
├── features.npy              # Нормализованные признаки (numpy)
├── NukeMatPredictor.pptx     # Презентация проекта
├── .gitignore
├── templates/
│   ├── index.html            # Главная страница (лендинг)
│   ├── form.html             # Страница с формой ввода
│   └── result.html           # Страница с результатами
├── static/
│   ├── style.css             # Стили для всех страниц
│   
└── docs/
    ├── user_stories.md
    ├── functional_requirements.md
    ├── non_functional_requirements.md
    ├── data_model.md
    ├── usecase.png
    ├── mockup.png
    ├── conceptual_model.md
    ├── er_diagram.jpg
    ├── testing_checklist.md
    └── screenshots.png
```

---

## Технологии

| Категория | Инструменты |
|---|---|
| Backend | Python, Flask |
| Frontend | HTML5, CSS3, Jinja2, Java Script |
| База данных | SQLite3, CSV |
| Графики | style.css, result.html |
| Управление версиями | Git, GitHub |

---

## Команда

| Участник | Роль |
|---|---|
| **Амир** | Product Leader + ML: координация, модель расчёта, презентация, помощь всем участникам команды |
| **Богдан** | Backend-разработчик: Flask, база данных, маршруты, интеграция |
| **Вероника** | Frontend + Дизайн: лендинг, форма, стили, мокапы|
| **Соня** | Frontend + Дизайн: страница результатов, график, адаптивность |
| **Саша** | Business Analyst + Docs: документация, координация |

---

## Документация

Вся проектная документация находится в папке `docs/`:

| Файл | Описание |
|---|---|
| [`conceptual_model.md`](docs/conceptual_model.md) | Концептуальная модель: слои системы и их взаимодействие |
| [`data_model.md`](docs/data_model.md) | Модель данных: поля таблицы, типы, ограничения |
| [`er_diagram.jpg`](docs/er_diagram.jpg) | ER-диаграмма базы данных |
| [`functional_requirements.md`](docs/functional_requirements.md) | Функциональные требования |
| [`non_functional_requirements.md`](docs/non_functional_requirements.md) | Нефункциональные требования (производительность, надёжность) |
| [`testing_checklist.md`](docs/testing_checklist.md) | Чеклист тестирования |
| [`user_stories.md`](docs/user_stories.md) | Пользовательские истории |
| [`mockup.png`](docs/mockup.png) | Мокапы страниц (дизайн до реализации) |
| [`screenshots.png`](docs/screenshots.png) | Скриншоты готового сайта |
| [`usecase.png`](docs/usecase.png) | Use-case диаграмма |

---

